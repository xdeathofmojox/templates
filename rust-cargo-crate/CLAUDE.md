...
Single Rust crate with Nix flakes for the dev environment.

Source lives under `src/`, Nix config under `nix/`.
...
- **Testing**: Integration tests live in `tests/`. Cover happy paths, edge cases, convergence behavior, and `#[should_panic]` for contract violations.
